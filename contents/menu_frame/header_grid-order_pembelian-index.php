...
$index["debug"] = 1;
$index["ajax"] 			= 0;
$index_navbutton["generate"] = "reload|add|import_excel";
$index["filter"]      = 1;
$index_filter["hide"] = 0;
...
$index_table["column"][$i]["sort"] 		= "spl.nama";
$index_table["column"][$i]["search"] 	= 1;
...
$index_table["column"][$i]["name"] 		= "departemen";
$index_table["column"][$i]["sort"] 		= "m4.nama";
$index_table["column"][$i]["search"] 	= 1;
$index_table["column"][$i]["caption"] 	= "Departement";
$index_table["column"][$i]["align"] 	= "";
$index_table["column"][$i]["width"] 	= "";
$i++;
...
$index["query_select"] = " 	SELECT
							...
							a.nomor,
							-- c.kode kodepraorder,
							-- c.kodepr kode_permintaan,	
							-- (SELECT GROUP_CONCAT(DISTINCT CONCAT('&#x25C8;&nbsp;', x.kodepr)) FROM tdbelipermintaanmaster a1 join thbelipermintaanmaster x on x.nomor = a1.nomorthbelipermintaanmaster WHERE a1.nomor = a.nomorthbelipermintaanmaster AND a1.status_aktif>0) kode_permintaan,
							m3.nama as pembuat,
							spl.nama as supplier,
							m2.nama as approve,
							a.status_aktif,
							m4.nama as departemen
							from thbeliorder a
							left JOIN mhrelasi spl ON spl.nomor = a.nomormhrelasi
							left join mhadmin m3 on m3.nomor = a.dibuat_oleh
							left join mhadmin m2 on m2.nomor = a.disetujui_oleh 
							join mhdepartemen m4 on m4.nomor = a.nomormhdepartemen
";
