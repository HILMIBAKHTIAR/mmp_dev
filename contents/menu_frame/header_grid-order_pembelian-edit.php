...
$qa = "
    SELECT 
      CONCAT(nomor, '|', nama) as departemen
    FROM mhdepartemen a 
...
$edit["field"][$i]["label"] = "Tanggal PR";
$edit["field"][$i]["input"] = "tanggal_pr";
$edit["field"][$i]["label_class"] = "req ";
$edit["field"][$i]["input_class"] = "required date_1";
// $edit["field"][$i]["input_attr"]["onchange"] = "load_grid_detail()";
$i++;
...
$edit["field"][$i]["browse_set"]["param_output"]     = array(
	"ppn|ppn", "top|top",
	"nomormhrelasikategori|browse_master_kategorisupplier_hidden",
	"kategori_supplier|browse_master_kategorisupplier_search",
	"kategori_supplier|kategori_supplier_terpilih",
);
...
$edit["field"][$i]["label_class"]                 = "required";
...
$edit["field"][$i]["input_element"] = "select1";
$edit["field"][$i]["input_option"] = $departments;
$edit["field"][$i]["input_attr"]["onchange"] = "load_grid_detail()";
$i++;
// $edit["field"][$i]["label"]                     = "Departemen";
// $edit["field"][$i]["label_class"]                 = "req";
...
// $edit["field"][$i]["browse_set"]["call_function"] = array("load_grid_detail", "calculate_all");
// $i++;
...
$edit["field"][$i]["label"]                     = "kategori_supplier_terpilih";
$edit["field"][$i]["input"]                        = "kategori_supplier_terpilih";
$edit["field"][$i]["input_save"]                        = "skip";
$edit["field"][$i]["label_class"]                 = "req";
$edit["field"][$i]["input_attr"]["readonly"]         = "readonly";
...
<script type="text/javascript">
	$(document).ready(function() {
		var select_departemen = $('#nomormhdepartemen').select2();
	});

	function filter_departemen() {
		var kategori = $('#kategori_supplier_terpilih').val()
		console.log({
			kategori
		})

		var departements = (<?= json_encode($departments) ?>)
		// format to id and text from split by '|'
		departements = departements.map(function(departement) {
			var split = departement.split('|')
			return {
				id: split[0],
				text: split[1]
			}
		...
		var bahan_baku_departements = departements.filter(function(departement) {
			return departement.text == 'PPIC' || departement.text == 'RND'
		})

		var non_bahan_baku_departements = departements.filter(function(departement) {
			return departement.text != 'PPIC' && departement.text != 'RND'
		})

		if (kategori == 'BAHAN BAKU') {
			// set select2 #nomormhdepartemen options to only 'PPIC' dan 'RND'
			$('#nomormhdepartemen').select2('destroy');
			$('#nomormhdepartemen').empty();
			$('#nomormhdepartemen').select2({
				data: bahan_baku_departements
			});
			...
			$('#nomormhdepartemen').empty();
			$('#nomormhdepartemen').select2({
				data: non_bahan_baku_departements
			});
		}
	...
	function load_grid_detail() {
		...
		var nomormhdepartemen = $("#nomormhdepartemen").val();
