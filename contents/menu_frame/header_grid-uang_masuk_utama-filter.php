...
if (!empty($_POST["relasinomor"])) {
	$customer = mysqli_fetch_array(mysqli_query($con, "SELECT * FROM mhrelasi WHERE nomor = $relasinomor"));
	$filtering["query"] .= " AND a.relasinomor = '" . $relasinomor . "' GROUP BY a.nomor";
}

$_SESSION["menu_" . $filtering["session_name"]]["filter_browse|relasinomor"] = $customer["kode"] . ' - ' . $customer['nama'] . "|" . $_POST["relasinomor"];
