...
//------------------------------------------------GRID DATA 1------------------------------------------------------
$i = 0;
$edit["detail"][$i]["table_name"] = "tdspkslitting";
...
$edit["field"][$i]["browse_setting"]             = "spk_slitting";
...
if ($edit["mode"] != "add") {
    ...
    if ($file_count > 0) {
        ...
        while ($row = mysqli_fetch_assoc($file_query)) {
            $json[] = $row;
            array_push($arr_images, $_SESSION["setting"]["dir_upload"] . "/" . str_replace("'", "\'", "SPK_SLITTING" . "/" . $json[$count_i]["nama"]));
...
if ($edit["mode"] != "add") {
    ...
    $edit_navbutton["field"][$i]["input_attr"]["onclick"]     =
        "link_confirmation('" . get_message(700) . "','?m=master_archieve&f=header_grid&&sm=edit&id=" . $r["nomor"] . "&table=" . $edit["table"] . "&tipe=SPK_SLITTING&menu=" . $_SESSION["g.menu"] . "&url=" . $_SESSION["g.url"] . "', '', 'Upload', 'fa fa-upload|btn-dark')";
    $i++;
}
...
<script type="text/javascript">
    ...
    function viewDataAction(nomor) {
        window.open("?m=spk_slitting_baru_detail&f=header_grid&&sm=edit&a=edit&no=" + nomor);
    }
