...
//------------------------------------------------GRID DATA 1------------------------------------------------------
$i = 0;
$edit["detail"][$i]["table_name"] = "tdqapsolventadhesive";
...
//------------------------------------------------GRID DATA 3------------------------------------------------------
$edit["detail"][$i]["table_name"] = "tdqapsolventadhesive_other";
$edit["detail"][$i]["field_name"] = array(
    "parameter",
    "media_cetak",
    "viscositas",
...
$edit["field"][$i]["input_class"]                 = "required";
$edit["field"][$i]["input_element"]             = "browse";
$edit["field"][$i]["browse_setting"]             = "master_barang_solventadhesive";
$i++;
...
if ($edit["mode"] != "add") {
	...
	WHERE 
		status_aktif > 0 AND 
		kategori = 'QA_SOLVENTADHESIVE' AND 
    ...
    if ($file_count > 0) {
        ...
        while ($row = mysqli_fetch_assoc($file_query)) {
            $json[] = $row;
            array_push($arr_images, $_SESSION["setting"]["dir_upload"] . "/" . str_replace("'", "\'", "QA_SOLVENTADHESIVE" . "/" . $json[$count_i]["nama"]));
...
if ($edit["mode"] != "add") {
    ...
    $edit_navbutton["field"][$i]["input_attr"]["onclick"]     =
        "link_confirmation('" . get_message(700) . "','?m=master_archieve&f=header_grid&&sm=edit&id=" . $r["nomor"] . "&table=" . $edit["table"] . "&tipe=QA_SOLVENTADHESIVE&menu=" . $_SESSION["g.menu"] . "&url=" . $_SESSION["g.url"] . "', '', 'Upload', 'fa fa-upload|btn-dark')";
    $i++;
}
...
<?php include "x_tab_data_images_incoming_solvent_adhesive.php"; ?>