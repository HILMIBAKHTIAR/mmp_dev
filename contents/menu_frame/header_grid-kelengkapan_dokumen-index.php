...
$index["query_select"] = " 	SELECT
							...
							(SELECT GROUP_CONCAT(DISTINCT CONCAT('&#x25C8;&nbsp;', COALESCE(CONCAT(h.kode, ' - ', h.nama_barang_customer), CONCAT(x.kode, ' - ', x.nama))) SEPARATOR '<br>') 
							FROM tdbeliorder a1 join thbeliorder t3 on t3.nomor = a1.nomorthbeliorder join mhbarang x on x.nomor = a1.nomormhbarang 
							LEFT JOIN thbarang h ON h.nomor = x.nomormhcilinder
							WHERE t3.nomor = a.nomorthbeliorder AND a1.status_aktif>0) nama_barang,
							m2.nama as supplier, 
							m3.nama as pembuat,
							m4.nama as approve,
							t.status_aktif 
...
$index["query_where"] .= "	AND a.nomor <> 0 AND a.status_disetujui = 1 ";
