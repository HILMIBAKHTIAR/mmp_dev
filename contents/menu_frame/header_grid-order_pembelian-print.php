...
$sql2 = "SELECT 
				...
				a.subtotal,
				m.nama satuan,
				COALESCE(CONCAT(h.nama_barang_customer), CONCAT(m2.nama)) AS nama_barang,
				COALESCE(h.kode, m2.kode) AS kode_barang
				FROM thbeliorder b
				join tdbeliorder a on a.nomorthbeliorder = b.nomor 
				join thbelipermintaanmaster c on c.nomor = b.nomorthbelipermintaanmaster
				join mhsatuan m on m.nomor = a.nomormhsatuanqty 
				join mhbarang m2 on m2.nomor = a.nomormhbarang 
				LEFT JOIN thbarang h ON h.nomor = m2.nomormhcilinder
