...
<script language="javascript" type="text/javascript">
    ...
    function autocomplete_barang(element) {
        $(element).on('keyup', function(e) {
            if (e.which === 113)
                browse_master_barang_pembelian_cylinder_varian_reloadopen();
        });
        return autocomplete_grid_<?= $grid_id ?>(element, 0, 'master_barang_pembelian_cylinder', 'master_barang_pembelian_cylinder', '', '');
    }
    ...
    function after_complete_<?= $grid_id ?>(rowid, cellname) {
        ...
        if (cellname == 'barang') {
            if (<?= $grid_id ?>_selected_suggest && <?= $grid_id ?>_selected_suggest.master_barang_pembelian_cylinder) {
                // SETTING OTHER FIELD
                jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'nomormhbarang', <?= $grid_id ?>_selected_suggest.master_barang_pembelian_cylinder.nomor);

                jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'barang', <?= $grid_id ?>_selected_suggest.master_barang_pembelian_cylinder.barang);
                jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'eta', <?= $grid_id ?>_selected_suggest.master_barang_pembelian_cylinder.tanggal_eta);
                jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'satuan_qty', <?= $grid_id ?>_selected_suggest.master_barang_pembelian_cylinder.satuan_qty);

                jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'jumlah_unit', <?= $grid_id ?>_selected_suggest.master_barang_pembelian_cylinder.qty);

                jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'keterangan', <?= $grid_id ?>_selected_suggest.master_barang_pembelian_cylinder.keterangan);
                jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'nomorthpemesanancylinder', <?= $grid_id ?>_selected_suggest.master_barang_pembelian_cylinder.nomorthpemesanancylinder);
                jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'nomorthbarang', <?= $grid_id ?>_selected_suggest.master_barang_pembelian_cylinder.nomorthbarang);
                jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'nomormhsatuanqty', <?= $grid_id ?>_selected_suggest.master_barang_pembelian_cylinder.nomormhsatuanqty);
