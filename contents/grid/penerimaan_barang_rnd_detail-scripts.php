...
<script language="javascript" type="text/javascript">
    ...
    function autocomplete_barang(element) {
        $(element).on('keyup', function(e) {
            if (e.which === 113)
                browse_master_barang_pembelian_rnd_varian_reloadopen();
        });
        return autocomplete_grid_<?= $grid_id ?>(element, 0, 'master_barang_pembelian_rnd', 'master_barang_pembelian_rnd', '', '');
    }
    ...
    function after_complete_<?= $grid_id ?>(rowid, cellname) {
        ...
        if (cellname == 'barang') {
            if (<?= $grid_id ?>_selected_suggest && <?= $grid_id ?>_selected_suggest.master_barang_pembelian_rnd) {
                // SETTING OTHER FIELD
                jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'nomormhbarang', <?= $grid_id ?>_selected_suggest.master_barang_pembelian_rnd.nomor);

                jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'barang', <?= $grid_id ?>_selected_suggest.master_barang_pembelian_rnd.barang);

                jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'satuan_qty', <?= $grid_id ?>_selected_suggest.master_barang_pembelian_rnd.satuan);

                jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'berat_jenis', <?= $grid_id ?>_selected_suggest.master_barang_pembelian_rnd.berat_jenis);

                jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'nomormhsatuanqty', <?= $grid_id ?>_selected_suggest.master_barang_pembelian_rnd.nomormhsatuanqtypurchasing);

                jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'nomormhsatuanberat', <?= $grid_id ?>_selected_suggest.master_barang_pembelian_rnd.nomormhsatuanberatpurchasing);
            }       
        }
