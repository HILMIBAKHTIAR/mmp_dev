...
function autocomplete_account(element)
{
	...
	{
		if(e.which === 113)
			browse_grid_master_biaya_uang_detail_reloadopen();
	});
...
function after_complete_<?=$grid_id?>(rowid,cellname)
{
	if(cellname == 'account')
	{
		...
		{
			jQuery(<?=$grid_id?>_element).jqGrid('setCell',rowid,'transaksi_nomor',<?=$grid_id?>_selected_suggest.master_account_data.transaksi_nomor);
			jQuery(<?=$grid_id?>_element).jqGrid('setCell',rowid,'transaksi_tabel',<?=$grid_id?>_selected_suggest.master_account_data.transaksi_tabel);
			jQuery(<?=$grid_id?>_element).jqGrid('setCell',rowid,'jenis',<?=$grid_id?>_selected_suggest.master_account_data.jenis);
		}
	}
	if(cellname == 'cabang')
	{
