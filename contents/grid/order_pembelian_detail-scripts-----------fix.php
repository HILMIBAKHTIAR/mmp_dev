...
<script language="javascript" type="text/javascript">
	...
	// ! RUMUS INCLUDE, EXCLUDE, NON PPN
	// function sum_subtotal_<?= $grid_id ?>() {
	// 	var subtotal = jQuery(<?= $grid_id ?>_element).jqGrid('getCol', 'subtotal', false, 'sum');
	// 	var uang_muka = $('#uang_muka').val() ?? 0;

	// 	var total_diskon = jQuery(<?= $grid_id ?>_element).jqGrid('getCol', 'subtotal_diskon', false, 'sum');
	...
	function calculate_diskon_and_subtotal(rowid, jumlah_beli) {
		var harga = parseFloat(jQuery(<?= $grid_id ?>_element).jqGrid('getCell', rowid, 'harga_satuan'));
		var jumlah_bayar = parseFloat(jQuery(<?= $grid_id ?>_element).jqGrid('getCell', rowid, 'jumlah_bayar'));
		var diskon_1 = parseFloat(jQuery(<?= $grid_id ?>_element).jqGrid('getCell', rowid, 'diskon_1'));
		var diskon_2 = parseFloat(jQuery(<?= $grid_id ?>_element).jqGrid('getCell', rowid, 'diskon_2'));
		var diskon_3 = parseFloat(jQuery(<?= $grid_id ?>_element).jqGrid('getCell', rowid, 'diskon_3'));
		var total_diskon_1 = 0
		var total_diskon_2 = 0
		var total_diskon_3 = 0
		var subtotal_setelah_diskon_2 = 0
		var subtotal_setelah_diskon_3 = 0
		if (isNaN(diskon_1)) diskon_1 = 0;
		if (isNaN(diskon_2)) diskon_2 = 0;


		var subtotal = jumlah_beli * harga;
		jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'subtotal', subtotal);

		var subtotal_sebelum_diskon_1 = parseFloat(jQuery(<?= $grid_id ?>_element).jqGrid('getCell', rowid, 'subtotal'));
		var total_diskon_1 = ((diskon_1 * harga) / 100);
		var subtotal_setelah_diskon_1 = subtotal_sebelum_diskon_1 - total_diskon_1;
		jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'diskon_1_nominal', total_diskon_1);

		if (diskon_2 > 0) {
			total_diskon_2 = ((diskon_2 * (harga - total_diskon_1)) / 100);
			subtotal_setelah_diskon_2 = subtotal_setelah_diskon_1 - total_diskon_2;
			jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'diskon_2_nominal', total_diskon_2);
		} else {
			total_diskon_2 = 0;
			subtotal_setelah_diskon_2 = subtotal_setelah_diskon_1 - total_diskon_2;
			jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'diskon_2_nominal', total_diskon_2);
		...
		if (diskon_3 > 0) {
			total_diskon_3 = ((diskon_3 * (harga - total_diskon_1 - total_diskon_2)) / 100);
			subtotal_setelah_diskon_3 = subtotal_setelah_diskon_2 - total_diskon_3;
			jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'diskon_3_nominal', total_diskon_3);
		} else {
			total_diskon_3 = 0;
			subtotal_setelah_diskon_3 = subtotal_setelah_diskon_2 - total_diskon_3;
			jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'diskon_3_nominal', total_diskon_3);
		...
		var netto = harga - ((total_diskon_1 ?? 0) + (total_diskon_2 ?? 0) + (total_diskon_3 ?? 0))

		jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'subtotal', jumlah_bayar * netto);
		jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'netto', netto);


		return (total_diskon_1 ?? 0) + (total_diskon_2 ?? 0) + (total_diskon_3 ?? 0);
	}
