...
<script language="javascript" type="text/javascript">
    function autocomplete_account(element) {
        ...
        return autocomplete_grid_<?= $grid_id ?>(element, 0, 'master_account_lain', 'master_account_lain', '', '');
    }

    function after_complete_<?= $grid_id ?>(rowid, cellname) {

        if (cellname == 'account') {
            if (<?= $grid_id ?>_selected_suggest && <?= $grid_id ?>_selected_suggest.master_account_lain) {
                // SETTING OTHER FIELD
                jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'nomormhaccount', <?= $grid_id ?>_selected_suggest.master_account_lain.nomor);
            }
        }
