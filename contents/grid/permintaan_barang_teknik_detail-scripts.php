...
<script language="javascript" type="text/javascript">
    ...
    function autocomplete_barang(element) {
        $(element).on('keyup', function(e) {
            if (e.which === 113)
                browse_master_barang_pembelian_teknik_varian_reloadopen();
        });
        return autocomplete_grid_<?= $grid_id ?>(element, 0, 'master_barang_pembelian_teknik', 'master_barang_pembelian_teknik', '', '');
    }
    ...
    function after_complete_<?= $grid_id ?>(rowid, cellname) {
        ...
        if (cellname == 'barang') {
            if (<?= $grid_id ?>_selected_suggest && <?= $grid_id ?>_selected_suggest.master_barang_pembelian_teknik) {
                // SETTING OTHER FIELD
                jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'nomormhbarang', <?= $grid_id ?>_selected_suggest.master_barang_pembelian_teknik.nomor);

                jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'barang', <?= $grid_id ?>_selected_suggest.master_barang_pembelian_teknik.barang);

                jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'satuan_qty', <?= $grid_id ?>_selected_suggest.master_barang_pembelian_teknik.satuan);

                jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'nomormhsatuanqty', <?= $grid_id ?>_selected_suggest.master_barang_pembelian_teknik.nomormhsatuanqty);

                jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'area', <?= $grid_id ?>_selected_suggest.master_barang_pembelian_teknik.area);
                
                jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'nomormhlokasi', <?= $grid_id ?>_selected_suggest.master_barang_pembelian_teknik.nomormhlokasi);
