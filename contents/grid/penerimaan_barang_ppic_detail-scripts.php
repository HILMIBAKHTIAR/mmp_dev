...
<script language="javascript" type="text/javascript">
    ...
    function autocomplete_barang(element) {
        $(element).on('keyup', function(e) {
            if (e.which === 113)
                browse_master_barang_pembelian_ppic_varian_reloadopen();
        });
        return autocomplete_grid_<?= $grid_id ?>(element, 0, 'master_barang_pembelian_ppic', 'master_barang_pembelian_ppic', '', '');
    }
    ...
    function autocomplete_area(element) {
        $(element).on('keyup', function(e) {
            if (e.which === 113)
                browse_master_area_varian_reloadopen();
        });
        return autocomplete_grid_<?= $grid_id ?>(element, 0, 'master_area', 'master_area', '', '');
    }
    ...
    function after_complete_<?= $grid_id ?>(rowid, cellname) {
        ...
        if (cellname == 'barang') {
            if (<?= $grid_id ?>_selected_suggest && <?= $grid_id ?>_selected_suggest.master_barang_pembelian_ppic) {
                // SETTING OTHER FIELD
                jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'nomormhbarang', <?= $grid_id ?>_selected_suggest.master_barang_pembelian_ppic.nomor);

                jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'barang', <?= $grid_id ?>_selected_suggest.master_barang_pembelian_ppic.barang);

                jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'satuan_qty', <?= $grid_id ?>_selected_suggest.master_barang_pembelian_ppic.satuan);

                jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'berat_jenis', <?= $grid_id ?>_selected_suggest.master_barang_pembelian_ppic.berat_jenis);

                jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'nomormhsatuanqty', <?= $grid_id ?>_selected_suggest.master_barang_pembelian_ppic.nomormhsatuanqtypurchasing);

                jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'nomormhsatuanberat', <?= $grid_id ?>_selected_suggest.master_barang_pembelian_ppic.nomormhsatuanberatpurchasing);
            }       
        }
        ...
        if (cellname == 'area') {
            if (<?= $grid_id ?>_selected_suggest && <?= $grid_id ?>_selected_suggest.master_area) {
                // SETTING OTHER FIELD
                jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'nomormhlokasi', <?= $grid_id ?>_selected_suggest.master_area.nomor);
            }    
        }
