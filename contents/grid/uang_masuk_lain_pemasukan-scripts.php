...
function autocomplete_pemasukan(element)
{
	$(element).on('keyup',function(e)
	{
		if(e.which === 113)
			browse_grid_master_pemasukan_uang_detail_reloadopen();
	});
	return autocomplete_grid_<?=$grid_id?>(element,0,'master_pemasukan_data','master_account_jenis_pemasukan','','');
	// return autocomplete_grid_<?=$grid_id?>(element,0,'master_pemasukan_data','master_account_lain_v2','','');
...
{
	if(cellname == 'pemasukan')
	{
		if(<?=$grid_id?>_selected_suggest && <?=$grid_id?>_selected_suggest.master_pemasukan_data)
		{
			jQuery(<?=$grid_id?>_element).jqGrid('setCell',rowid,'transaksi_nomor',<?=$grid_id?>_selected_suggest.master_pemasukan_data.transaksi_nomor);
			jQuery(<?=$grid_id?>_element).jqGrid('setCell',rowid,'transaksi_tabel',<?=$grid_id?>_selected_suggest.master_pemasukan_data.transaksi_tabel);
			jQuery(<?=$grid_id?>_element).jqGrid('setCell',rowid,'jenis',<?=$grid_id?>_selected_suggest.master_pemasukan_data.jenis);
			jQuery(<?=$grid_id?>_element).jqGrid('setCell',rowid,'nomormhaccount',<?=$grid_id?>_selected_suggest.master_pemasukan_data.nomor
		}
	}
	if(cellname == 'cabang')
	{
