...
<script language="javascript" type="text/javascript">
    ...
    function after_save_cell_<?= $grid_id ?>(rowid, cellname) {
        ...
        var nomormhrelasikelompok = jQuery(<?= $grid_id ?>_element).jqGrid('getCell', rowid, 'nomormhrelasikelompok');
        var pic = jQuery(<?= $grid_id ?>_element).jqGrid('getCell', rowid, 'pic');

        if (pic) {
			jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'nomormhrelasikelompok', 2);
		} else {
			jQuery(<?= $grid_id ?>_element).jqGrid('setCell', rowid, 'nomormhrelasikelompok', 2);
		}
