        ...
        if ($edit["submit_function"] == "") {
            ...
            $edit["submit_form"] = "
                ...
                \t\tvar obj  = $.dialog({
                    ...
                    \t\tonOpen: function () { ";
            if ($edit["period"] == 1) {
                if (!isset($check_approval_fields["tanggal"])) {
                    $date_field = $_SESSION["setting"]["field_tanggal"];
                    $date_format= $_SESSION["setting"]["date"];
                } else {
                    $date = explode("|", $check_approval_fields["tanggal"]);
                    if (empty($date[0])) 
                        $date_field = $_SESSION["setting"]["field_tanggal"];
                    else 
                        $date_field = $date[0]; 
                ...
                $edit["submit_form"] .= "
                    ...
                    \t\t\tvar formData = new FormData($('#form_" . $edit["id"] . "')[0]);
                    ...
                    \t\t\t$.ajax({
                        \t\t\turl :'".$config["webspira"]."codes/period.php',
                        \t\t\ttype: 'POST',
                        \t\t\tdata: formData,
                        \t\t\tprocessData: false,
                        \t\t\tcontentType: false,
                        \t\t\tsuccess: function(message) {
                            ...
                            \t\t\telse " . $edit["ajax_submit"] ."
                        \t\t\t}
                    \t\t\t});";    
            }
            else
                $edit["submit_form"] .= $edit["ajax_submit"];       
            $edit["submit_form"] .= "\n\t\t\t\t\t\t\t}\n\t\t\t\t\t\t});";
